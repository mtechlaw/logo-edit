...
with st.sidebar:
    ...
    default_optical = st.toggle("Optical centering", value=True)
    default_x = st.slider("Manual X nudge", -60, 60, 0)
    default_y = st.slider("Manual Y nudge", -60, 60, 0)
    st.markdown("**Output spec**")
    st.code("356 x 346 px canvas\n346 px circle\ntransparent outside circle", language=None)
...
if "logo_file_settings_v4" not in st.session_state:
    st.session_state.logo_file_settings_v4 = {}
...
if uploads:
    ...
    for idx, uploaded in enumerate(uploads):
        key = file_key(uploaded)
        if key not in st.session_state.logo_file_settings_v4:
            st.session_state.logo_file_settings_v4[key] = {
                "threshold": default_threshold,
                "logo_scale": default_scale,
                "x_offset": default_x,
                "y_offset": default_y,
                "optical_centering": default_optical,
            }

        settings_dict = st.session_state.logo_file_settings_v4[key]
        ...
        with cols[idx % 4]:
            ...
            try:
                original = load_image_from_bytes(data, uploaded.name)
            except Exception as e:
                st.error(f"Could not open {uploaded.name}: {e}")
                continue

            with st.expander("Adjust logo", expanded=False):
                ...
                settings_dict["optical_centering"] = st.toggle(
                    f"Optical centering · {uploaded.name}", value=bool(settings_dict["optical_centering"]), key=f"oc-{key}"
                )
            ...
            st.session_state.logo_file_settings_v4[key] = settings_dict
    ...
    if rendered:
        zip_path = Path("/tmp/branded_logos_v4.zip")
        batch_zip(rendered, zip_path)
        st.markdown("---")
        st.download_button(
            "Download all as ZIP",
            data=zip_path.read_bytes(),
            file_name="branded_logos_v4.zip",
            mime="application/zip",
            use_container_width=True,
        )
else:
    st.info("Upload your provider logos to generate polished branded badges.")
